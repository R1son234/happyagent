...
- 显式状态：循环步骤、工具调用、资源读取、Skill 注入都必须可观测。
...
5. 加载用户指定或默认 Skill。
6. 由 Skill 构造增强后的 system prompt 和可用能力集合。
7. Engine 启动 ReAct Loop。
8. LLM 返回下一步动作：
   - 直接回答。
   - 请求调用工具。
9. Engine 执行工具并记录 observation。
10. 继续循环直到：
   - 得到 `final_answer`
   - 遇到不可恢复错误
   - 达到最大步数
...
- 当前 MCP、Tool Schema、Skill 注入已经带来足够多的系统复杂度。
...
- `loader.go`
  - 扫描目录、校验 Skill。
- `injector.go`
  - 将 Skill 配置转换为运行时 prompt 和能力限制。

#### 验证要求

- 能加载一个 demo skill。
- 引用不存在 Tool 时给出清晰错误。
- Skill 注入后 system prompt 明确可见。
...
- 每层都先做最小接口和最小实现。
- MCP 先只支持最小可验证能力。
- Skill 先只做声明式 prompt + capability 注入。
- 尽量用结构化输出而不是自由文本协议。
