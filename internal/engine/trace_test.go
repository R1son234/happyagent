...
func TestBuildRunTraceAggregatesUsageAndToolCalls(t *testing.T) {
	...
	if trace.ExecutedToolCallCount != 2 || trace.SuccessfulToolCallCount != 1 {
		t.Fatalf("unexpected execution counts: %+v", trace)
	}
	if trace.ExecutedToolCallsByName["file_list"] != 1 || trace.ExecutedToolCallsByName["file_read"] != 1 {
		t.Fatalf("unexpected executed tool calls: %+v", trace.ExecutedToolCallsByName)
	}
